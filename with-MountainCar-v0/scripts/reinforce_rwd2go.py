...
def normalize_state(state, env_name):
    if env_name == "MountainCar-v0":
        # state = [position, velocity]
        min_position, max_position = -1.2, 0.6
        min_velocity, max_velocity = -0.07, 0.07
        norm_position = (state[0] - min_position) / (max_position - min_position)
        norm_velocity = (state[1] - min_velocity) / (max_velocity - min_velocity)
        return np.array([norm_position, norm_velocity], dtype=np.float32)
    else:
        return state

def reinforce_rwd2go_2(env, policy, optimizer, n_episodes=1000, max_t=200, gamma=1.0, print_every=100, row_target=5, max_reward=-110, min_reward=-200):
    target_achieved_row = 0
    half_reward = 0.5 * (max_reward + min_reward) 
    intermediate_mean_found = False
    intermediate_max_found = False
    intermediate_step_max, intermediate_step_mean, converged_step = 0, 0, 0 
    env_name = env.spec.id

    scores_deque = deque(maxlen=100)
    scores = []
    for e in range(1, n_episodes):
        state = normalize_state(env.reset(), env.spec.id)
        saved_log_probs, rewards = [], []
        ...
        for t in range(max_t):
            ...
            state = normalize_state(next_state, env.spec.id)
            rewards.append(reward)
            if done:
                break
