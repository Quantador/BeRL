...
def reinforce_rwd2go_2(env, policy, optimizer, early_stop=False, n_episodes=1000, max_t=1000, gamma=1.0, print_every=100, row_target=5, near_max_reward=-80):
    target_achieved_row = 0
    if near_max_reward >= 0:
    ...
    for e in range(1, n_episodes):
        ...
        if (not intermediate_mean_found) and ep_return <= half_reward and np.mean(scores[-10:]) >= half_reward:
